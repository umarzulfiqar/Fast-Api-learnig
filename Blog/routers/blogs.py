...
def create_blog(request:scheme.Blog,db:Session=Depends(get_db)):
    return blog_actions.create_blog(request,db,)

#read data
@router.get('/',response_model=list[scheme.show_blog])
def read_blog(db:Session=Depends(get_db),):
    return blog_actions.read_blog(db)

@router.get('/{id}',response_model=scheme.show_blog)
def read_with_id(response:Response,id,db:Session=Depends(get_db)):
    return blog_actions.read_blog_with_id(db,id)


#updata data
@router.put('/{id}',status_code=status.HTTP_202_ACCEPTED)
def update_blog(id,request:scheme.Blog,db:Session=Depends(get_db)):
